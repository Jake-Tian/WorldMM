...
from worldmm.llm import PromptTemplateManager, generate_text_response
...
class HippoRAG:
    ...
    def __init__(self,
        ...
        if self.global_config.openie_mode == 'online':
            self.openie = OpenIE(model_name=self.global_config.llm_name)
        ...
        if embedding_model is None:
            self.embedding_model = EmbeddingModel(text_model_name=self.global_config.embedding_model_name)
        else:
    ...
    def index(self, docs: List[str]):
        ...
        self.chunk_embedding_store.insert_strings(docs, batch_size=self.global_config.embedding_batch_size)
        ...
        self.entity_embedding_store.insert_strings(entity_nodes, batch_size=self.global_config.embedding_batch_size)

        logger.info(f"Encoding Facts")
        fact_texts = [str(fact) for fact in facts]
        self.fact_embedding_store.insert_strings(fact_texts, batch_size=self.global_config.embedding_batch_size)

        logger.info(f"Constructing Graph")

        if not hasattr(self, 'node_to_node_stats'):
    ...
    def retrieve(self,
        ...
        if gold_docs is not None:
            ...
            logger.warning("RetrievalRecall is not available. Evaluation skipped.")
            pass
    ...
    def merge_openie_results(self,
                             ...
                             ner_results_dict: Dict[str, NerOutput],
                             triple_results_dict: Dict[str, TripleOutput]) -> List[dict]:
        """
        ...
        Parameters:
            ...
            ner_results_dict (Dict[str, NerOutput]): A dictionary mapping chunk keys
                to their corresponding NER extraction results.
            triple_results_dict (Dict[str, TripleOutput]): A dictionary mapping chunk
        ...
        for chunk_key, row in chunks_to_save.items():
            ...
            try:
                chunk_openie_info = {'idx': chunk_key, 'passage': passage,
                                 'extracted_entities': ner_results_dict[chunk_key],
                                 'extracted_triples': triple_results_dict[chunk_key]}
            ...
            if not (len(self.passage_node_keys) == len(ner_results_dict) == len(triple_results_dict)):
                ...
                for chunk_id in self.passage_node_keys:
                    if chunk_id not in ner_results_dict:
                        ner_results_dict[chunk_id] = NerOutput(
                            chunk_id=chunk_id,
                            metadata={},
                            unique_entities=[]
                        )
                    if chunk_id not in triple_results_dict:
                        triple_results_dict[chunk_id] = TripleOutput(
                            chunk_id=chunk_id,
                            metadata={},
                            triples=[]
                        )
    ...
    def get_query_embeddings(self, queries: List[str] | List[QuerySolution]):
        ...
        if len(all_query_strings) > 0:
            ...
            query_embeddings_for_triple = self.embedding_model.encode(all_query_strings, batch_size=self.global_config.embedding_batch_size)
            for query, embedding in zip(all_query_strings, query_embeddings_for_triple):
                self.query_to_embedding['triple'][query] = embedding

            logger.info(f"Encoding {len(all_query_strings)} queries for query_to_passage.")
            query_embeddings_for_passage = self.embedding_model.encode(all_query_strings, batch_size=self.global_config.embedding_batch_size)
    ...
    def get_fact_scores(self, query: str) -> np.ndarray:
        ...
        if query_embedding is None:
            query_embedding = self.embedding_model.encode(query, batch_size=self.global_config.embedding_batch_size)
    ...
    def update(self, docs: List[str], **kwargs):
        self.ready_to_retrieve = False
        self.index(docs, **kwargs)
